...
| **employees/roles** | #17 ✅ List · #18 ✅ Add · #19 ✅ Edit · #20 Permissions · #21 User Log | `users` ✅ + `roles` ✅ · `permissions` + `user-logs` ยังไม่ทำ | #17 ไม่มี _components · #18/#19 ✅: `EmployeeFormFields` (ใช้ร่วม) + `employeeForm.ts` (pure) — **ตัดออก:** AvatarUploader/ToggleRow/PasswordShuffleButton/EmployeeSummaryCard/MetaChips/DangerZone (ทำฟอร์มพื้นฐานตามแพทเทิร์น Add Product ก่อน) · #20: RoleList, PermissionCollapseSection/CheckboxGroup · #21: `DetailDrawer` (reuse) | `employees` ✅ (+ `saved`/`saveFailed`, `nav.employeesAdd`/`employeesEdit`, `validation.email`) · `permissions` ยังว่าง · **#21 unlock แท็บ "ประวัติการสต็อก" ของ #6** |
